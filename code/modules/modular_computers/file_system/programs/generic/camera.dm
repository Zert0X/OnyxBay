...
/datum/nano_module/camera_monitor/New(host, topic_manager)
	...
	// Keep all per-slot lists dense (1..6) to avoid numeric index runtimes.
	...
	for(var/i = 1, i <= CAMERA_MULTI_SLOT_COUNT, i++)
		...
		var/map_ref = "camera_[safe_ref]_[i]_map_a"
		map_refs[i] = map_ref
...
/datum/nano_module/camera_monitor/tgui_interact(mob/user, datum/tgui/ui)
	...
	update_active_camera_screens(TRUE)
...
/datum/nano_module/camera_monitor/tgui_data(mob/user)
	...
	data["multi_layouts"] = list(CAMERA_LAYOUT_1X2, CAMERA_LAYOUT_2X2, CAMERA_LAYOUT_2X3)
	...
	data["map_ref_single"] = length(map_refs) >= 1 ? map_refs[1] : null
	data["map_z_levels"] = map_z_levels
	data["world_max_x"] = world.maxx
	data["world_max_y"] = world.maxy
	data["holomap_images"] = get_holomap_images(map_z_levels)
	data["holomap_width"] = 480
	data["holomap_height"] = 480
	data["holomap_offset_x"] = HOLOMAP_OFFSET_X
	data["holomap_offset_y"] = HOLOMAP_OFFSET_Y
...
/datum/nano_module/camera_monitor/proc/update_active_camera_screens(force = FALSE)
	ensure_slot_lists_ready()
	for(var/i = 1, i <= CAMERA_MULTI_SLOT_COUNT, i++)
		var/obj/machinery/camera/camera_to_render = null

		if(view_mode == CAMERA_VIEW_MODE_MULTI)
			camera_to_render = get_slot_camera(i)
		else if(i == 1)
			camera_to_render = current_camera
...
/datum/nano_module/camera_monitor/proc/update_slot_screen(slot, obj/machinery/camera/C, force = FALSE)
	if(slot < 1 || slot > CAMERA_MULTI_SLOT_COUNT)
		return
	if(slot > length(cam_screens) || slot > length(cam_backgrounds))
		return
...
/datum/nano_module/camera_monitor/proc/show_camera_static(slot)
	...
	cam_background.maptext = "<div style='font-weight:bold;color:#9ca3af;text-align:center;'>NO SIGNAL</div>"
	cam_background.fill_rect(1, 1, DEFAULT_CAMERA_MAP_SIZE, DEFAULT_CAMERA_MAP_SIZE)
...
/datum/nano_module/camera_monitor/proc/ensure_slot_lists_ready()
	if(length(multi_slots) < CAMERA_MULTI_SLOT_COUNT)
		multi_slots = list(null, null, null, null, null, null)
	if(length(map_refs) < CAMERA_MULTI_SLOT_COUNT)
		map_refs = list(null, null, null, null, null, null)
