      ...
      >
        ...
        >
          ...
          {!visibleMapCameras.length && (
            <Box className="CameraConsole__mapEmpty">No cameras on this Z-level.</Box>
          )}
...
export const CameraConsole = (props, context) => {
  ...
  const [modalSlot, setModalSlot] = useLocalState(context, "modalSlot", 0);
  const [modalSearch, setModalSearch] = useLocalState(context, "modalSearch", "");
  const [mapZ, setMapZ] = useLocalState(context, "mapZ", 0);
  const [mapZoom, setMapZoom] = useLocalState(context, "mapZoom", 1);
  const [mapPanX, setMapPanX] = useLocalState(context, "mapPanX", 0);
  const [mapPanY, setMapPanY] = useLocalState(context, "mapPanY", 0);
  ...
  const handleOpenSingle = (camera: CameraData) => {
    act("switch_camera", { camera: camera.camera });
    act("set_view_mode", { mode: VIEW_MODE_SINGLE });
  };
  ...
  const openModalForSlot = (slot: number) => {
    setModalSlot(slot);
    setModalSearch("");
  };

  const modalCameras = filterCameras(cameras, modalSearch);
  const selectedSlotData = getSlotData(data.multi_slots, modalSlot);
  const singleMapRef = getMapRef(data.map_refs, 1) || data.map_ref_single || null;

  return (
    <Window width={1220} height={820} title="Camera Monitoring" resizable>
      <Window.Content className="CameraConsole" scrollable={false}>
        <Stack fill className="CameraConsole__root">
          <Stack.Item basis="300px" grow={false} shrink={0}>
          ...
          <Stack.Item grow>
            <Stack fill vertical className="CameraConsole__main">
              ...
              {data.view_mode === VIEW_MODE_SINGLE && (
                <Stack.Item grow>
                  ...
                  >
                    {singleMapRef ? (
                      <ByondUi
                        className="CameraConsole__singleViewport"
                        params={{ id: singleMapRef, type: "map" }}
                      />
                    ) : (
                      <NoticeBox>No viewport is available.</NoticeBox>
                    )}
                  </Section>
                </Stack.Item>
              )}
              ...
              {data.view_mode === VIEW_MODE_MAP && (
                <Stack.Item grow>
                  <Stack fill className="CameraConsole__mapMode">
                    ...
                    <Stack.Item grow>
                      <Section fill title="Live Preview">
                        {singleMapRef ? (
                          <ByondUi
                            className="CameraConsole__mapPreview"
                            params={{ id: singleMapRef, type: "map" }}
                          />
                        ) : (
                          <NoticeBox>No viewport is available.</NoticeBox>
                      ...
                      >
                        {Array.from({ length: visibleSlots }, (_, idx) => idx + 1).map(
                          (slot) => {
                            ...
                            const slotMapRef =
                              getMapRef(data.map_refs, slot) ||
                              (slot === 1 ? singleMapRef : null);
                            return (
                              <Box
                                key={slot}
                                className={classes([
                                ...
                                onClick={() => act("set_active_slot", { slot })}
                                onDblClick={() =>
                                  slotCamera && act("open_slot_single", { slot })
                                }
                              ...
                              >
                                ...
                                <Box className="CameraConsole__slotViewportWrap">
                                  {slotMapRef ? (
                                    <ByondUi
                                      ...
                                      params={{ id: slotMapRef, type: "map" }}
                                    />
                                  ) : (
                                    <NoticeBox>No viewport.</NoticeBox>
                                  )}
                                </Box>
                                ...
                                <Box className="CameraConsole__slotFooter">
                                  {slotCamera ? (
                                    ...
                                    <>
                                      <span className="CameraConsole__muted">
                                        Empty -&gt; Select Camera
                                      </span>
                                      <Button
                                        icon="camera"
        ...
        {!!modalSlot && (
          <Modal className="CameraConsole__modal">
            <Section
              title={`Select Camera for Slot ${modalSlot}`}
              buttons={
                <Button icon="xmark" color="bad" onClick={() => setModalSlot(0)} />
              }
            >
              <Input
                fluid
                ...
                value={modalSearch}
                onInput={(_, value: string) => setModalSearch(value)}
              />
              <Box className="CameraConsole__modalList">
                {modalCameras.map((camera) => (
                  <Button
                    key={camera.camera}
                    fluid
                    color={camera.deact ? "average" : undefined}
                    onClick={() => {
                      act("assign_camera", {
                        slot: modalSlot,
                ...
                {!modalCameras.length && (
                  <NoticeBox>No cameras found.</NoticeBox>
                )}
              </Box>
              {selectedSlotData?.camera?.name && (
                <Box mt={1} className="CameraConsole__muted">
                  Current: {selectedSlotData.camera.name}
                </Box>
              )}
            </Section>
          </Modal>
